...
export function apply(ctx) {
  ...
  const dispatch = defineTool({
    ...
    description:
      ...
      '闲聊/纯对话/只读查询不要调用，直接回答即可。任何「动手」操作（执行命令/读写改文件/联网/推送/发布/批量/产出保存）都必须经我派子代理，前台不许自己干。P/M 级会先弹三要素确认卡（userQuestions.ask）问主人，主人点「准了」才执行——这个确认卡独立于 DSH 的 Full access/审批开关，Full access 下也照样弹，别拿审批开关当不弹卡的理由。幂等：同一 idempotency_key 不重复派发。',
    parameters: {
